...
function Required(name) {
    function required() {
        throw new Error(ERR_REQUIRED + '`' + name + '`');
    }
...
function Conflict(name) {
    function conflict() {
        throw new Error(ERR_CONFLICT + '`' + name + '`');
    }
...
function toTrait(properties) {
    if (properties instanceof Trait)
        return properties;
...
var TraitProto = Trait.prototype = _create(Trait.prototype, {
     ...
     */
    resolve: {
        value: function resolve(resolutions) {
            var renames    = {},
                exclusions = [],
